...
export function Headers<C extends Koa.Context, R = any>(key: string, ...piper: TClassIndefiner<PipeLineTransform<string, R>>[]): ParameterDecorator;
export function Headers<C extends Koa.Context, R = any>(key?: string | TClassIndefiner<PipeLineTransform<string, any>>, ...piper: TClassIndefiner<PipeLineTransform<any, any>>[]) {
  ...
  return ParameterMetaCreator.define<C, R>(async ctx => {
    ...
    if (typeof key === 'string') {
      if (!piper.length) return ctx.headers[key] as R;
